...
PORTS=(8020 6381 15672 8050)  # Don't touch PostgreSQL or Redis core ports (5432/6379)
...
echo "🧹 Rebuilding Docker containers without deleting volumes..."
docker-compose down  # ❌ Don't use `-v` to keep DB data
...
docker exec aidocumines_web python manage.py migrate || {
    echo "❌ ERROR: Failed to apply Django migrations!";
    exit 1;
}

echo "✅ All services are up and database schema is synced!"
